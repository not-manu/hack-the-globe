...
const multiMaterialSchema = z.object({
  items: z
    .array(materialSchema)
    .describe(
      'Array of all distinct construction materials detected in the image. Return one entry per material type. If only one material is visible, return an array with one item.',
    ),
})
...
export default async function handler(
  ...
  try {
    const result = await generateText({
      ...
      output: Output.object({ schema: multiMaterialSchema }),
      messages: [
        {
          role: 'user',
          content: [
